package com.smartjam.analyzer.infrastructure.visualizer;
...
import com.smartjam.analyzer.domain.port.DebugVisualizer;
import lombok.extern.slf4j.Slf4j;
import org.springframework.context.annotation.Profile;
import org.springframework.stereotype.Component;

/**
 * ImageIO implementation of {@link DebugVisualizer}. Generates a scientific-grade heatmap using the Magma color palette
 * and log-scale normalization.
 */
@Component
@Profile("debug")
@Slf4j
public class ImageIoDebugVisualizer implements DebugVisualizer {
    ...
    public void generateHeatmap(AnalysisResult result, String filename) {
        ...
        if (path == null || path.isEmpty()) {
            log.warn("Warping path is null or empty, skipping path rendering");
        } else {
            g.setColor(Color.WHITE);
