...
/**
 * Performance evaluator using Dynamic Time Warping (DTW) and Cosine Similarity.
 * Provides granular scoring for pitch and rhythm, along with time-aligned feedback events.
 */
public class DtwPerformanceEvaluator implements PerformanceEvaluator
{
            ...
            List<AnalysisResult.FeedbackEvent> feedbacks)
    {
        ...
        ErrorState pitchES = new ErrorState("Wrong note");
        ErrorState rhythmES = new ErrorState("Wrong rhythm");
        ...
        for (int[] p : path)
        {
            ...
            double pitchSev = Math.clamp((dist - PITCH_THRESHOLD) / (1.0 - PITCH_THRESHOLD), 0.0,
                    1.0);
            double rhythmSev = Math.clamp(
                    (drift - RHYTHM_TOLERANCE_RATIO) / (0.5 - RHYTHM_TOLERANCE_RATIO), 0.0, 1.0);

            processErrorState(pitchES, feedbacks, tRef, tStud, pitchSev, dist > PITCH_THRESHOLD);
            processErrorState(rhythmES, feedbacks, tRef, tStud, rhythmSev,
                    drift > RHYTHM_TOLERANCE_RATIO);
        }
    ...
    private AnalysisResult buildFinalResult(
            PathMetrics metrics, double[][] matrix, List<int[]> path,
            List<AnalysisResult.FeedbackEvent> feedbacks)
    {
        ...
        return new AnalysisResult(
                Math.sqrt(pScore * rScore), pScore, rScore, path, feedbacks,
     ...
     * Handles silence vs silence as a perfect match (0.0 distance).
     */
    double[][] computeCostMatrix(List<float[]> ref, List<float[]> stud)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            for (int j = 0; j < m; j++)
            {
                ...
                else dtw[i][j] = Math.min(dtw[i - 1][j],
                            Math.min(dtw[i][j - 1], dtw[i - 1][j - 1])) + dist;
            }
        }
        return dtw;
    }
    ...
    private static class ErrorState
    {
        ...
        AnalysisResult.FeedbackEvent flush()
        {
            ...
            AnalysisResult.FeedbackEvent event = null;

            if (rEnd - rStart >= MIN_ERROR_DURATION_SEC || sEnd - sStart >= MIN_ERROR_DURATION_SEC)
            {
                event = new AnalysisResult.FeedbackEvent(rStart, rEnd, sStart, sEnd, message,
                        maxSev);
            }
