...
/**
 * ImageIO implementation of {@link DebugVisualizer}.
 * Generates a scientific-grade heatmap using the Magma color palette and log-scale normalization.
 */
@Component
@Profile("debug")
@Slf4j
public class ImageIoDebugVisualizer implements DebugVisualizer
{
    ...
    public void generateHeatmap(AnalysisResult result, String filename)
    {
        ...
        BufferedImage img =
                new BufferedImage(plotW + marginL + marginR, plotH + marginT + marginB,
        ...
        double maxVal = allValues.length > 0 ? allValues[(int) (allValues.length * 0.95)] : 1.0;
        ...
        for (int j = 0; j <= m; j += tickStepX)
        {
            ...
            g.drawLine(x, marginT + plotH, x, marginT + plotH + 10);
        ...
        g.drawString("STUDENT TIMELINE (Frames) ->", marginL + plotW / 2 - 200,
                marginT + plotH + 90);
    ...
    {
        if (ratio < 0.25f)
            return lerpColor(new Color(0, 0, 5), new Color(60, 15, 110), ratio / 0.25f);
        if (ratio < 0.50f) return lerpColor(new Color(60, 15, 110), new Color(190, 40, 60),
                (ratio - 0.25f) / 0.25f);
        if (ratio < 0.75f) return lerpColor(new Color(190, 40, 60), new Color(250, 150, 40),
                (ratio - 0.50f) / 0.25f);
        return lerpColor(new Color(250, 150, 40), new Color(255, 255, 180),
                (ratio - 0.75f) / 0.25f);
    }
