...
class RoomRepository(
    ...
    suspend fun syncAssignmentsPage(connectionId: UUID, page: Int, size: Int): Result<AssignmentPageInfo> {
        return try {
            val response = assignmentsApi.getAssignmentsByConnection(connectionId, page = page, size = size)
            if (response.isSuccessful && response.body() != null) {
                val body = response.body()!!
                val existing = assignmentDao.getAssignmentsByIds(body.content.map { it.id })
                    .associateBy { it.id }

                val entities = body.content.map { dto ->
    ...
    suspend fun ensureAssignmentDetailsCached(assignmentId: UUID): Result<AssignmentEntity> {
        return try {
            val existing = assignmentDao.getAssignmentById(assignmentId)
                ?: return Result.failure(Exception("Assignment not found in cache"))

            val response = assignmentsApi.getAssignment(assignmentId)
            if (response.isSuccessful && response.body() != null) {
                val dto = response.body()!!
                Log.d("RoomRepository", "Fetched assignment details: id=${dto.id} description=${dto.description?.take(100)} referenceAudioUrl=${dto.referenceAudioUrl}")
                val localPath = cacheReferenceAudioIfNeeded(existing, dto)
                Log.d("RoomRepository", "Reference audio localPath for assignment ${existing.id}: $localPath")
    ...
    suspend fun createAssignment(request: CreateAssignmentRequest): Result<AssignmentUploadResponse> {
        return try {
            ...
            if (response.isSuccessful && response.body() != null) {
                ...
                Log.w("RoomRepository", "createAssignment failed: code=${response.code()} body=$errorBody")
    ...
    suspend fun syncSubmissions(assignmentId: UUID): Result<Unit> {
        return try {
            ...
            if (response.isSuccessful && response.body() != null) {
                val body = response.body()!!
                val existing = submissionResultDao.getResultsForAssignmentOnce(assignmentId).associateBy { it.id }
    ...
    suspend fun createSubmission(assignmentId: UUID): Result<SubmissionUploadResponse> {
        return try {
            ...
            if (response.isSuccessful && response.body() != null) {
                ...
                val errorBody = response.errorBody()?.string()
                Log.w("RoomRepository", "createSubmission failed: code=${response.code()} body=$errorBody")
    ...
    suspend fun getSubmissionResult(submissionId: UUID, assignmentId: UUID): Result<SubmissionResultResponse> {
        return try {
            val response = submissionsApi.getSubmissionResult(submissionId)
            if (response.isSuccessful && response.body() != null) {
                val dto = response.body()!!
                val created = java.time.Instant.now()
                val existing = submissionResultDao.getResultsForAssignmentOnce(assignmentId).associateBy { it.id }[dto.id]
                submissionResultDao.insertAll(listOf(
                    SubmissionResultEntity(
                        id = dto.id,
                        assignmentId = assignmentId,
                        status = dto.status.name,
    ...
    suspend fun cacheSubmissionAudioIfNeeded(submissionId: UUID, assignmentId: UUID, urlString: String?): Result<String?> = withContext(Dispatchers.IO) {
        try {
            if (urlString.isNullOrBlank()) {
                return@withContext Result.success(null)
            }

            val existing = submissionResultDao.getResultsForAssignmentOnce(assignmentId).associateBy { it.id }[submissionId]
            ...
            val updated = SubmissionResultEntity(
                id = submissionId,
                assignmentId = assignmentId,
    ...
    suspend fun uploadFileToS3(uploadUrl: String, file: File): Result<Unit> = withContext(Dispatchers.IO) {
        try {
            val uri = java.net.URI(uploadUrl)
            val originalHost = if (uri.port == -1) uri.host else "${uri.host}:${uri.port}"

            val fixedUrl = uploadUrl
                .replace("localhost", "10.0.2.2")
                .replace("127.0.0.1", "10.0.2.2")
            ...
            httpClient.newCall(request).execute().use { response ->
                if (response.isSuccessful) {
                    ...
                    Log.w("RoomRepository", "uploadFileToS3 failed: code=${response.code} body=$errorBody")
    ...
    private suspend fun cacheReferenceAudioIfNeeded(
        ...
        val url = dto.referenceAudioUrl.toString().takeIf { it.isNotBlank() }
